	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出预售票账务汇总数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/export?ShanghuCode=410000001");
						$("#searchForm").submit();
						$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/getCsDay");
					}
				},{buttonsFocus:1});
		...
		function upload(day){
			top.$.jBox.confirm("确认要下载'"+day+"'账单吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/trade/vShanghuTrade/exportGzDay?day="+day+"&shanghu=410000001");
					$("#searchForm").submit();
					$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/getCsDay");
				}
			},{buttonsFocus:1});
...
<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/account/tAccountSingleAmount/getCsDay">预售票账务汇总日报表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="tAccountSingleAmount" action="${ctx}/account/tAccountSingleAmount/getCsDay" method="post" class="breadcrumb form-search">
...
<li>
	<div class="header">
		<span class="label"><fmt:formatDate value="${tAccountSingleAmount.tradeDate}" pattern="yyyy-MM-dd"/> </span>
              ...
              <span class="label5"><a href="#" onclick="upload('<fmt:formatDate value="${tAccountSingleAmount.tradeDate}" pattern="yyyy-MM-dd"/>')">下载日账单</a></span>
