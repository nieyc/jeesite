	...
	<script type="text/javascript">
		...
		$(document).ready(function() {
			$("#btnExport").click(function(){
				top.$.jBox.confirm("确认要导出预售票账务汇总数据吗？","系统提示",function(v,h,f){
					if(v=="ok"){
						$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/export?ShanghuCode=510000001");
						$("#searchForm").submit();
						$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/getGzDay");
					}
				},{buttonsFocus:1});
		...
		function upload(day){
			top.$.jBox.confirm("确认要下载'"+day+"'账单吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/trade/vShanghuTrade/exportGzDay?day="+day+"&shanghu=510000001");
					$("#searchForm").submit();
					$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/getGzDay");
				}
			},{buttonsFocus:1});
...
<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/account/tAccountSingleAmount/getGzDay">预售票账务汇总日报表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="tAccountSingleAmount" action="${ctx}/account/tAccountSingleAmount/getGzDay" method="post" class="breadcrumb form-search">
