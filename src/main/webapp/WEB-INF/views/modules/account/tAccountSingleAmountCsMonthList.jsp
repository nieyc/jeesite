...
$(document).ready(function(){
	...
	/* 	$(document).ready(function() {
		...
		function uploadAll(){
			top.$.jBox.confirm("确认要导出预售票账务汇总数据吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/exportMonth?ShanghuCode=410000001");
					$("#searchForm").submit();
		...
		function upload(day){
			top.$.jBox.confirm("确认要下载'"+day+"'账单吗？","系统提示",function(v,h,f){
				if(v=="ok"){
					$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/exportDay?day="+day+"&ShanghuCode=410000001");
					$("#searchForm").submit();
					$("#searchForm").attr("action","${ctx}/account/tAccountSingleAmount/getCsMonth");
				}
			},{buttonsFocus:1});
...
<ul class="nav nav-tabs">
		<li class="active"><a href="${ctx}/account/tAccountSingleAmount/getCsMonth">预售票账务汇总月报表</a></li>
	</ul>
	<form:form id="searchForm" modelAttribute="tAccountSingleAmount" action="${ctx}/account/tAccountSingleAmount/getCsMonth" method="post" class="breadcrumb form-search">
